...
/**
 * Callback feedable to jg_context_load, allowing to extend context when
 * creating it (adding new classes).
 *
 ...
 * @param parent_id  Id of the parent class, or @ref jg_id_none if the class
 * have no parent.
 * @param size       Size of the object this new class wraps. See @ref
 * jg_class_new.
 */
 ...
 *
 * @param jg_error_handler The callback that will be called when an error
 * occurs.
 */
 ...
 * This will call the callback previously set through jg_set_error_handler, or
 * the default error handler, that will printf the message to stdout if none was
 * set.
 *
 * @param error_code Error code enum.
 * @param format     Message format, will be passed to sprintf with the
