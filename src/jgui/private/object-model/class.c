...
void jg_class_add_method(jg_class* class_, const char* name, jg_method method) {
	...
	// TODO(corentin@ki-dour.org) : handle case where member of the same name is
	// already registered
...
void jg_class_init(jg_class* class_, const jg_class* parent, size_t size) {
	*class_ = (jg_class) {.member_index = {0}, .parent = parent, .size = size};
...
static void item_cleanup(void* item) {
	jg_member_cleanup(item);
}
