 ...
 * @param item_size Size of an item in the array. Generally use
 * sizeof(stored_type).
 */
jg_vector jg_vector_init(size_t item_size);

/**
 * Helper macro to initialize a vector with item_size set as the size of the
 * given type.
 *
 ...
 *
 * @param item Pointer to the item to add. The item will be copied in the
 * vector. If the memory pointed by item is smaller than the vector's item size,
 ...
 * If index is not in the bounds of the underlying array, the behavior is
 * undefined.
 *
