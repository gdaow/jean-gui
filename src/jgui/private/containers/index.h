 ...
 * @param item_size Size of an item in the index. Generally use
 * sizeof(stored_type).
 */
 ...
 *
 * If the index was packed through jg_index_pack, the provided buffer will not
 * be free. Only memory allocated via jg_index_add will be free.
 *
 * @param item_cleanup Cleanup callback that will be called for every item in
 * the index.
 */
 ...
 * @param item The item value, that will be copied in the index. You can pass
 * NULL, and assign the item value through the returned pointer.
 *
 * @return Pointer to the item in the index. You can write this to set the item
 * value through it instead of through the item parameter if more convenient.
 */
