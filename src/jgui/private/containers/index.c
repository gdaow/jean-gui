...
size_t jg_index_packed_size(const jg_index* index) {
    assert(index != NULL);
    return index->_count * (index->_item_size + sizeof(size_t)) + index->_key_buffer_count;
}
...
static char* get_key_buffer(const jg_index* index) {
    ...
    return ((char*)index->_buffer + (index->_item_size + sizeof(size_t)) * index->_size);
}
...
static void index_grow(jg_index* index, size_t new_key_size) {
    ...
    if(size != index->_size || key_buffer_size != index->_key_buffer_size) {
        size_t new_buffer_size = size * (index->_item_size + sizeof(size_t)) + key_buffer_size;
