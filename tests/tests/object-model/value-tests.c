...
static void check_is_functions(jg_value value,
                               jg_check_function_t expected_true_check) {
...
static void test_value_int(void** state) {
	...
	const int test_int =
	    42; // If bugs happen with edge cases, put them in regression tests.
...
/** jg_double, jg_is_double and jg_to_double should correctly handle boolean
 * values. */
...
/** jg_string, jg_is_string and jg_to_string should correctly handle boolean
 * values. */
...
/** jg_object, jg_is_object and jg_to_object should correctly handle boolean
 * values. */
static void test_value_object(void** state) {
	(void)state;
	/*
	 * TODO
	jg_context* context = jg_context_load((jg_plugin[]) { user_model_plugin,
	NULL }); const jg_class* admin_class = jg_context_get_class(context,
	admin_class_id()); void* test_object = jg_object_new(admin_class);
...
jg_begin_tests(NULL, NULL)
	cmocka_unit_test(test_value_bool), cmocka_unit_test(test_value_int),
	    cmocka_unit_test(test_value_double),
	    cmocka_unit_test(test_value_string), cmocka_unit_test(test_value_raw),
