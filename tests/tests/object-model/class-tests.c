...
static void get_property(jg_class* class_,
...
/** Properties should be correctly registered in class, and retrieved, including
 * overriden ones. */
static void test_class_add_property(void** state) {
	...
	jg_class_add_property(
	    &parent_class, "parent_property", &parent_getter, NULL);
	jg_class_add_property(
	    &parent_class, "overriden_property", &parent_getter, NULL);

	jg_class_init(&child_class, &parent_class, sizeof(char));
	jg_class_add_property(&child_class, "child_property", &child_getter, NULL);
	jg_class_add_property(
	    &child_class, "overriden_property", &child_getter, NULL);
...
static void call_method(jg_class* class_,
...
/** Methods should be correctly registered in class, and retrieved, including
 * overriden ones. */
...
static void test_class_duplicated_member_fail(void** state) {
	...
	expect_assert_failure(
	    jg_class_add_method(&class_, "member", &child_method));
	expect_assert_failure(
	    jg_class_add_property(&class_, "member", &child_getter, NULL));
